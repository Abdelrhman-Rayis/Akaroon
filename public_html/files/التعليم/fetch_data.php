...
if(isset($_POST["action"]))
{
	...
	$edument = $connect->prepare($query);
	$edument->execute();
	$result = $edument->fetchAll();
	$total_row = $edument->rowCount();
	$output = '';
	if($total_row > 0)
	{
		foreach($result as $row)
		{
			...
			<div class="col-sm-4 col-lg-3 col-md-3">
				...
				<a href="files/'. $row['id'] .'.pdf">	<img style=" align:center; margin-bottom:16px; width:auto;height:250px;" src="image/'. $row['image'] .'">       </a>
					<p align="center"><strong><a href="files/'. htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8') .'.pdf'>'. htmlspecialchars($row['The_Title_of_Paper_Book'], ENT_QUOTES, 'UTF-8') .'</a></strong></p>
