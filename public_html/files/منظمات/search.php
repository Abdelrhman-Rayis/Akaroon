  ...
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="ak-sidebar">
        ...
        <!-- Author filter -->
        <?php
        $q = $connect->prepare("SELECT DISTINCT(The_number_of_the_Author) FROM org ORDER BY The_number_of_the_Author");
        $q->execute();
        $authors = $q->fetchAll();
        if (!empty($authors)):
        ?>
        ...
        <?php
        $q = $connect->prepare("SELECT DISTINCT(Field_of_research) FROM org ORDER BY Field_of_research");
        $q->execute();
        $fields = $q->fetchAll();
        if (!empty($fields)):
        ?>
        ...
        <?php
        $q = $connect->prepare("SELECT DISTINCT(Place_of_issue) FROM org ORDER BY Place_of_issue");
        $q->execute();
        $places = $q->fetchAll();
        if (!empty($places)):
        ?>
