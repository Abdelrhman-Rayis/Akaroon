...
<head>
  ...
  <title>السياسة — مكتبة عكارون</title>
...
<div class="ak-page-header">
  <h2>السياسة</h2>
  <p>العلوم السياسية والعلاقات الدولية</p>
</div>
  ...
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="ak-sidebar">
        ...
        <!-- Author filter -->
        <?php
        $q = $connect->prepare("SELECT DISTINCT(The_number_of_the_Author) FROM pol ORDER BY The_number_of_the_Author");
        $q->execute();
        $authors = $q->fetchAll();
        if (!empty($authors)):
        ?>
        ...
        <?php
        $q = $connect->prepare("SELECT DISTINCT(Field_of_research) FROM pol ORDER BY Field_of_research");
        $q->execute();
        $fields = $q->fetchAll();
        if (!empty($fields)):
        ?>
        ...
        <?php
        $q = $connect->prepare("SELECT DISTINCT(Place_of_issue) FROM pol ORDER BY Place_of_issue");
        $q->execute();
        $places = $q->fetchAll();
        if (!empty($places)):
        ?>
